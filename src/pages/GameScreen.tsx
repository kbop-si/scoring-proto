...
const initialUI: UIState = {
  batAdvOpen: false, batOutOpen: false, runAdvOpen: false, runOutOpen: false,
  fielderOpen: false, deflOpen: false, playersOpen: false,
  batAdvResult: null, batAdvBallType: null, batAdvHitData: null, batOutType: null, runAdvResult: null, runAdvDest: null, runAdvEarned: true as boolean | 'half', runAdvRbi: false, runAdvPitcher: '', runAdvFielder: null, runOutResult: null,
  stkOpen: false, stkPitchType: null,
  selRunnerBase: null,
  fielderSeq: [], fielderTitle: '',
  deflFielder: null, deflType: '라이나',
  playerSelIdx: null, playerList: [], playersTitle: '',
  luSelIdx: null, benchSelIdx: null, luTeam: 'away',
  srchName: '', srchNum: '',
};
...
export default function GameScreen({ setup, onEnd }: Props) {
  ...
  const [baseTargets, setBaseTargets] = useState<BaseTargetState>({ active: false, fromBase: '', onSelect: null });
  ...
  const handleBaseTargetClick = useCallback((dest: string) => {
  ...
  const selRunnerBadge = UI.selRunnerBase && G.runners[UI.selRunnerBase]
    ? `[${UI.selRunnerBase}: ${G.runners[UI.selRunnerBase]!.name}]`
    : '';

  // ── Pitch ─────────────────────────────────────────────────────────────────
  const handlePitch = useCallback((pitchType: PitchType) => {
    const cell = G.cells[G.selCellKey];
    if (cell?.result) { showToast('이미 결과가 기록된 타석입니다'); return; }

    const willK = (pitchType === 'S' || pitchType === 'SW' || pitchType === 'BS') && G.strikes >= 2;
    ...
    if (pitchType === 'FE') {
      ...
      setUI((p) => ({ ...p, fielderOpen: true, fielderTitle: '파울실책 수비수', fielderSeq: [] }));
      return;
    }
  ...
  const confirmBatAdv = useCallback(() => {
    if (!UI.batAdvResult && !UI.batAdvHitData) { showToast('진루 사유 선택 필요'); return; }
  ...
  const autoConfirmBatAdv = useCallback((result: string, ballType?: '땅' | '뜬' | '라', hitData?: import('../types').HitData) => {
  ...
  const confirmStrikeout = useCallback((result: string) => {
    ...
    if (result === 'K') {
      ...
      if (newOuts >= 3) showToast(`3아웃! ${G.inning}회 ${G.half === 'top' ? '초' : '말'} 종료 — [다음이닝] 버튼 클릭`);
    } else {
      const bat = curLU[G.curBatterOrder - 1];
      const msg: Record<string, string> = { KW: '폭투 낫아웃', KP: '포일 낫아웃', KE: '실책 낫아웃' };
      showToast(`${bat?.name || '타자'} ${msg[result] || result} 1루`);
    }
  }, [UI.stkPitchType, G, curLU, dispatch, showToast]);

  const applyBatOutResult = useCallback((result: string, dp?: boolean, tp?: boolean) => {
  ...
  const openRunAdv = useCallback(() => {
    if (!UI.selRunnerBase || !G.runners[UI.selRunnerBase]) {
      showToast('다이아몬드에서 주자를 먼저 클릭하세요'); return;
    }
    setUI((p) => ({ ...p, runAdvOpen: true, runAdvResult: null, runAdvDest: null, runAdvEarned: true as boolean | 'half', runAdvFielder: null, runAdvRbi: false, runAdvPitcher: '' }));
  }, [UI.selRunnerBase, G.runners, showToast]);

  const handleRunnerDestClick = useCallback((dest: Base | 'HOME') => {
    if (!UI.selRunnerBase || !G.runners[UI.selRunnerBase]) return;
    setUI((p) => ({ ...p, runAdvOpen: true, runAdvDest: dest, runAdvResult: null, runAdvEarned: true as boolean | 'half', runAdvFielder: null, runAdvRbi: false, runAdvPitcher: '' }));
  }, [UI.selRunnerBase, G.runners]);

  const confirmRunAdv = useCallback(() => {
    if (!UI.runAdvResult) { showToast('진루 사유 선택'); return; }
    if (!UI.runAdvDest) { showToast('목적 베이스 선택'); return; }
    if (!UI.selRunnerBase || !G.runners[UI.selRunnerBase]) { showToast('주자가 없습니다'); return; }
    ...
    const isSteal = typeof reason === 'string' && (reason.includes('도루') || reason.includes('스틸'));
    const advCode = typeof reason === 'string' ? getAdvCode(reason) : undefined;
    setUI((p) => ({ ...p, runAdvOpen: false, selRunnerBase: null, runAdvDest: null, runAdvResult: null, runAdvFielder: null, runAdvRbi: false, runAdvPitcher: '' }));
    dispatch({ type: 'RUN_ADV', base, runner, dest, earned, rbi, scorePitcher, steal: isSteal || undefined, advCode });

    if (dest === 'HOME') showToast(`${runner.name} 득점! (${earned === true ? '자책ER' : earned === 'half' ? '반자책' : '비자책UER'})`);
  ...
  const confirmRunOut = useCallback((fielderSeq: number[]) => {
    if (!UI.runOutResult) { showToast('아웃 사유 선택'); return; }
    if (!UI.selRunnerBase || !G.runners[UI.selRunnerBase]) { showToast('주자가 없습니다'); return; }
  ...
  const confirmDefl = useCallback(() => {
    if (!UI.deflFielder) { showToast('수비수 선택'); return; }
    setUI((p) => ({ ...p, deflOpen: false }));
    showToast(`디플렉션: ${UI.deflFielder}번(${UI.deflType})`);
  }, [UI, showToast]);

  // ── Substitution ─────────────────────────────────────────────────────────
  const handleFielderClick = useCallback((e: React.MouseEvent, pos: number) => {
    e.stopPropagation();
  ...
  const openSubst = useCallback((pos: number) => {
    ...
    setUI((p) => ({ ...p, playersOpen: true, playersTitle: `${POS_NAME[pos]} 교체`, playerList: [...bench], playerSelIdx: null }));
  }, [G, dispatch, showToast]);
  ...
  const openPitcherChange = useCallback(() => {
    ...
    setUI((p) => ({ ...p, playersOpen: true, playersTitle: '투수 교체', playerList: [...bench], playerSelIdx: null }));
  }, [G, dispatch, showToast]);

  const confirmPlayerSel = useCallback(() => {
    if (UI.playerSelIdx === null) { showToast('선수 선택'); return; }
    setUI((p) => ({ ...p, playersOpen: false }));
    if (playerCbRef.current) playerCbRef.current(UI.playerList[UI.playerSelIdx]);
  }, [UI, showToast]);

  const openRunnerSubst = useCallback((base: import('../types').Base) => {
    ...
    setUI((p) => ({ ...p, playersOpen: true, playersTitle: `대주자 교체 (${base})`, playerList: [...bench], playerSelIdx: null }));
  }, [G, dispatch, showToast]);
  ...
  const openBatterSubst = useCallback(() => {
    ...
    setUI((p) => ({ ...p, playersOpen: true, playersTitle: '대타 교체', playerList: [...bench], playerSelIdx: null }));
  }, [G, dispatch, showToast]);

  // ── Runner toggle ─────────────────────────────────────────────────────────
  const toggleRunnerSel = useCallback((base: Base) => {
    // pendingBatter 상태일 때: 클릭 즉시 RunAdv 모달 오픈
    if (G.pendingBatter && G.runners[base]) {
      setUI((prev) => ({ ...prev, selRunnerBase: base, runAdvOpen: true, runAdvResult: null, runAdvDest: null, runAdvEarned: true as boolean | 'half', runAdvRbi: false, runAdvPitcher: '' }));
      return;
    }
    setUI((prev) => {
  ...
  useEffect(() => {
    if (!G.pendingBatter) { prevPendingRef.current = null; return; }
  ...
  const handleNextInning = useCallback(() => {
    ...
    showToast(`${G.half === 'top' ? G.inning : G.inning + 1}회 ${G.half === 'top' ? '말' : '초'} 시작`);
  }, [G, dispatch, showToast]);

  const handleUndo = useCallback(() => {
    if (!G.history.length) { showToast('되돌릴 내용 없음'); return; }
    dispatch({ type: 'UNDO' });
    setUI((p) => ({ ...p, selRunnerBase: null }));
    showToast('Undo 완료');
  ...
  return (
    <div className="screen active" id="s-game">
              ...
              <div className="lineup-section">
                ...
                <div className="lineup-scroll">
                  <table className="lineup-tbl">
                    <thead>
                      <tr>
                        <th>#</th><th>수</th><th>선수</th>
                        <th>#</th><th>수</th><th>선수</th>
                      </tr>
                    </thead>
                    <tbody>
                      {Array.from({ length: Math.max(awayBatters.length, homeBatters.length) }, (_, i) => {
                        ...
                        const rowCls = isCurAway ? 'bat-cur-away' : isCurHome ? 'bat-cur-home' : '';
                        return (
      ...
      <BatAdvModal
        ...
        onSelect={(v, bt) => setUI((p) => ({ ...p, batAdvResult: v, batAdvBallType: bt ?? null, batAdvHitData: null }))}
      ...
      <RunAdvModal
        ...
        onClose={() => setUI((p) => ({ ...p, runAdvOpen: false, runAdvDest: null, runAdvFielder: null, runAdvRbi: false, runAdvPitcher: '' }))}
      />
