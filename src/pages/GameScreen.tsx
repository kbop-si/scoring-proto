...
import {
  ...
  MemoInputModal,
  GameDelayModal,
  WarningEjectionModal,
  UmpireChangeModal,
  MemoListModal,
} from '../components/modals/SpecialEventsModal';
...
export default function GameScreen({ setup, onEnd }: Props) {
  ...
  // DELETE_CELL 후 재입력 중: pendingBatter를 빈 베이스 자유 배치 허용
  ...
  // SheetWindow 팝업에서 보낸 타석 삭제 요청 수신
  useEffect(() => {
    const handler = (e: StorageEvent) => {
      if (e.key === 'kbo_delete_request' && e.newValue) {
        dispatchDeleteCell(e.newValue);
        localStorage.removeItem('kbo_delete_request');
      }
      if (e.key === 'kbo_sel_cell_request' && e.newValue) {
        dispatch({ type: 'SEL_CELL', key: e.newValue });
        localStorage.removeItem('kbo_sel_cell_request');
      }
    };
  ...
  const [checkSwingOpen, setCheckSwingOpen] = useState(false);
  const [memoInputOpen, setMemoInputOpen] = useState(false);
  const [gameDelayOpen, setGameDelayOpen] = useState(false);
  const [warningEjectionOpen, setWarningEjectionOpen] = useState(false);
  const [umpireChangeOpen, setUmpireChangeOpen] = useState(false);
  ...
  const handleEditRow = useCallback(
    (info: EditRowInfo) => {
      ...
      } else if (info.kind === 'batter_edit' || info.kind === 'pitcher_edit') {
        setEditRowInfo(info);
      } else {
        setEditRowInfo(info);
      }
  ...
  // 현재 수비팀 투수 이름(number) — RunAdv 자책점 기본값용
  const currentDefPitcherStr = useMemo(() => {
    const side = G.half === 'top' ? 'home' : 'away';
    const lu = side === 'home' ? G.homeLineup : G.awayLineup;
    const p = lu.find((x) => x.pos === 1);
    return p ? `${p.name}(${p.num})` : '';
  }, [G.half, G.homeLineup, G.awayLineup]);
  ...
  const autoConfirmBatAdv = useCallback(
    (
      ...
      setUI((p) => ({ ...p, batAdvOpen: false, batAdvResult: null, batAdvHitData: null }));
      // 주자가 있으면 chainCausedBy 설정 (연속플레이 표시용)
      const hasRunners = Object.values(G.runners).some(Boolean);
      ...
      if (chain && hitData) {
        ...
        if (dest) {
          ...
          // 연속플레이 유발 타자 저장 (BAT_ADV 전에 캡처 — 이후 curBatterOrder 증가)
        ...
        // 실책/기타 진루(E·FC·#·ob·DP_E·E기록·번트 등) — 1B 기본 + 연속플레이 활성화
        const dest: Base = '1B';
  ...
  const openRunAdv = useCallback(() => {
    ...
    setUI((p) => ({
      ...
      runAdvFielder: null,
      runAdvRbi: false,
      runAdvPitcher: currentDefPitcherStr,
    }));
  }, [UI.selRunnerBase, G.runners, showToast, guardThreeOut, currentDefPitcherStr]);

  // ── Chain-pending 타자 클릭 (blocked 상태에서 chain 타자 먼저 전진) ────────
  const handleChainPendingClick = useCallback(() => {
    if (!chainPendingBase) return;
    // chainTransit: 주자가 연속플레이으로 이동 중 (pendingBatter 없음)
  ...
  }, [chainPendingBase, G.pendingBatter, chainTransit, currentDefPitcherStr]);
  ...
  const handleRunnerDestClick = useCallback(
    ...
    [UI.selRunnerBase, G.runners, currentDefPitcherStr]
  );
  ...
  const confirmRunAdv = useCallback(
    (
      ...
      // chain 연속플레이 또는 자력진루(도루/폭투/포일/보크) + 목적 베이스에 기존 주자 있음
      ...
      // (사용자가 명시적으로 연속플레이 체크한 경우에만 — chainCausedBy 자동 적용 X)
      setChainBases((prev) => {
      ...
      dispatch({
        ...
        causedBy: chain ? (chainCausedBy ?? undefined) : undefined,
        // 작은 화살표는 사용자가 명시적으로 연속플레이 체크한 경우에만
        chain,
        deflection,
      });
  ...
  const toggleRunnerSel = useCallback(
    (base: Base) => {
      // 연속플레이 중인 주자: 일반 주자와 동일하게 선택 → 목적지 클릭 → 모달
  ...
  useEffect(() => {
    ...
    if (!blocked && !chainPendingBase) {
      ...
      if (!chainPendingBase) {
        // 1루 막힘: 빈 베이스 클릭으로 배치 가능하도록 baseTargets 활성화
        showToast('빈 루를 클릭하여 타자를 배치하세요');
        setBaseTargets({
          active: true,
          fromBase: 'pending',
          onSelect: (base) => {
            if (!G.runners[base as Base]) {
              dispatch({ type: 'CHAIN_BATTER_SKIP', toBase: base as Base, earned: true });
            }
            setBaseTargets({ active: false, fromBase: '', onSelect: null });
          },
        });
  ...
  }, [G.pendingBatter, G.runners, chainPendingBase, dispatch, showToast, setBaseTargets]);
    ...
    <div className="screen active" id="s-game">
      <div className="game-wrap">
        <Scoreboard
          ...
          onEditGameInfo={() => setGameInfoEditOpen(true)}
        />
        ...
        {showSheet ? (
          ...
          <div className="game-body view-main">
            ...
            <InputPanel
              ...
              onEnd={handleEnd}
              onVideoReview={() => setVideoReviewOpen(true)}
              onCheckSwing={() => setCheckSwingOpen(true)}
              onMemoInput={() => setMemoInputOpen(true)}
              onGameDelay={() => setGameDelayOpen(true)}
              onWarningEjection={() => setWarningEjectionOpen(true)}
              onUmpireChange={() => setUmpireChangeOpen(true)}
              onMemoList={() => setMemoListOpen(true)}
            />
      ...
      <EditRowModal
        ...
        onPinchHitter={(cellKey, player, mid) => {
          ...
          dispatch({ type: 'RETRO_SUBST_BATTER', side, cellKey, player, mid });
          setEditRowInfo(null);
        }}
        onPitcherChange={(side, inning, half, order, player, mid) => {
          dispatch({ type: 'RETRO_PITCHER_CHANGE', side, inning, half, order, player, mid });
          setEditRowInfo(null);
        }}
        getBench={(side) => (side === 'away' ? G.awayBench : G.homeBench)}
      />
      <BatAdvModal
        open={UI.batAdvOpen}
        ...
        return (
          <>
            <VideoReviewModal
              ...
              onClose={() => setVideoReviewOpen(false)}
              onSave={saveEv}
            />
            <CheckSwingModal
              G={G}
              open={checkSwingOpen}
              onClose={() => setCheckSwingOpen(false)}
              onSave={saveEv}
            />
            <MemoInputModal
              G={G}
              open={memoInputOpen}
              onClose={() => setMemoInputOpen(false)}
              onSave={saveEv}
            />
            <GameDelayModal
              G={G}
              open={gameDelayOpen}
              onClose={() => setGameDelayOpen(false)}
              onSave={saveEv}
            />
            <WarningEjectionModal
              G={G}
              open={warningEjectionOpen}
              onClose={() => setWarningEjectionOpen(false)}
              onSave={saveEv}
            />
            <UmpireChangeModal
              G={G}
              open={umpireChangeOpen}
              onClose={() => setUmpireChangeOpen(false)}
              onSave={saveEv}
            />
          </>
        );
      ...
      {gameInfoEditOpen && (
        <div
          ...
          onClick={() => setGameInfoEditOpen(false)}
        >
          <div onClick={(e) => e.stopPropagation()}>
            <GameInfoScreen
              setup={{
                ...
                awayBench: G.awayBench,
                homeBench: G.homeBench,
                stadium: G.stadium,
                gameNum: G.gameNum,
                startTime: G.startTime,
                endTime: G.endTime,
                attendance: G.attendance,
                umpireHome: G.umpireHome,
                umpire1B: G.umpire1B,
                umpire2B: G.umpire2B,
                umpire3B: G.umpire3B,
                umpireLeft: G.umpireLeft,
                umpireRight: G.umpireRight,
                umpireStandby: G.umpireStandby,
                recorder1: G.recorder1,
                recorder2: G.recorder2,
                temperature: G.temperature,
                humidity: G.humidity,
                windDir: G.windDir,
                windSpeed: G.windSpeed,
                weatherLog: G.weatherLog,
              }}
              onConfirm={(info) => {
                dispatch({ type: 'UPDATE_GAME_INFO', info });
                setGameInfoEditOpen(false);
                showToast('경기 정보를 수정했습니다');
              }}
              onBack={() => setGameInfoEditOpen(false)}
            />
          </div>
        </div>
      )}
