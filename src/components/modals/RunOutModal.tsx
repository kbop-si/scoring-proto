...
export default function RunOutModal({ open, runnerBase, runnerName, selected, onSelect, onConfirm, onClose }: Props) {
  ...
  const reset = () => {
    setSeq([]); setWInsert(false); setPInsert(false); setCsRecord(false); setPickoffBase('1');
  };
  const handleClose   = () => { reset(); onClose(); };
  const handleConfirm = () => { onConfirm(seq); reset(); };

  const addPos = (pos: number) => setSeq((prev) => prev.length < 4 ? [...prev, pos] : prev);

  const B = (v: string, l: string) => (
    <button key={v} className={`r-btn${selected === v ? ' sel' : ''}`} onClick={() => onSelect(v)}>{l}</button>
  );
    ...
    <div className={`ov${open ? ' open' : ''}`} id="ov-run-out">
      <div className="ov-card" style={{ minWidth: 580 }}>
        <div className="modal-title">
          주자 아웃 — <span style={{ color: 'var(--red)' }}>{runnerBase}: {runnerName}</span>
        </div>
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid var(--border2)' }}>
          ...
          <div style={{ width: 300, borderRight: '1px solid var(--border2)', padding: '10px 8px' }}>
            <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--text3)', marginBottom: 4, textAlign: 'center' }}>
              수비 위치 클릭
            </div>
                ...
                return (
                  <g key={pos} style={{ cursor: canAdd ? 'pointer' : inSeq ? 'default' : 'not-allowed' }}
                    ...
                    <circle cx={x} cy={y} r={15}
                      fill={inSeq ? '#1d4ed8' : canAdd ? 'rgba(255,255,255,0.22)' : 'rgba(255,255,255,0.07)'}
                      stroke={inSeq ? '#60a5fa' : canAdd ? 'rgba(255,255,255,0.7)' : 'rgba(255,255,255,0.2)'}
                      strokeWidth={inSeq ? 2 : 1.5}
                    />
                    {inSeq && (
                      <text x={x} y={y - 2} textAnchor="middle" fontSize="10" fontWeight="700" fill="#fff"
                        style={{ pointerEvents: 'none' }}>{SEQ_LABEL[idx]}</text>
                    )}
                    <text x={x} y={y + (inSeq ? 9 : 4)} textAnchor="middle"
                      fontSize={inSeq ? '8' : '12'} fontWeight="700"
                      fill={inSeq ? '#bfdbfe' : canAdd ? '#fff' : 'rgba(255,255,255,0.3)'}
                      style={{ pointerEvents: 'none' }}>{pos}</text>
                    {!inSeq && (
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 }}>
              <span style={{
                flex: 1, fontSize: 13, fontWeight: 700, fontFamily: 'monospace', textAlign: 'center',
                color: seq.length ? 'var(--blue)' : 'var(--text3)',
              }}>
                {seq.length ? seq.join(' → ') : '—'}
              </span>
              <button onClick={() => setSeq((s) => s.slice(0, -1))}
                style={{ padding: '2px 6px', fontSize: 10, border: '1px solid var(--border)', background: '#fff', borderRadius: 2 }}>
                ←
              </button>
          ...
          <div style={{ flex: 1, borderRight: '1px solid var(--border2)' }}>
            ...
            <div style={{ borderBottom: '1px solid var(--border2)' }}>
              <div className="rg">
                ...
                <button className={`r-btn${selected === 'A 1루터치' ? ' sel' : ''}`}
                  style={{ gridColumn: '1 / -1' }} onClick={() => onSelect('A 1루터치')}>
                  A  1루 터치 아웃
                </button>
                {B('B 2루터치', 'B  2루 터치 아웃')}
                {B('B 2루공과', 'B  2루 공과')}
          ...
          <div style={{ width: 110, padding: '10px 10px', display: 'flex', flexDirection: 'column', gap: 8 }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, cursor: 'pointer' }}>
              <input type="checkbox" checked={wInsert} onChange={() => setWInsert((v) => !v)}
                style={{ accentColor: 'var(--blue)' }} />
              폭투삽입
            </label>
            <label style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, cursor: 'pointer' }}>
              <input type="checkbox" checked={pInsert} onChange={() => setPInsert((v) => !v)}
                style={{ accentColor: 'var(--blue)' }} />
              포일삽입
            </label>
            <label style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, cursor: 'pointer' }}>
              <input type="checkbox" checked={csRecord} onChange={() => setCsRecord((v) => !v)}
                style={{ accentColor: 'var(--blue)' }} />
              도루자기록
            </label>
            <div style={{ borderTop: '1px solid var(--border2)', paddingTop: 8, marginTop: 2 }}>
              ...
              {(['1', '2', '3'] as PickoffBase[]).map((b) => (
                <label key={b} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 10, cursor: 'pointer', marginBottom: 2 }}>
                  <input type="radio" name="runOutPickoff" checked={pickoffBase === b}
                    onChange={() => setPickoffBase(b)} style={{ accentColor: 'var(--blue)' }} />
                  {b}루견제
                </label>
              ))}
            </div>
        ...
        <div className="modal-footer">
          <button className="btn-ok" onClick={handleConfirm}>확인</button>
