...
export default function StrikeoutModal({ open, pitchType, defLU, onConfirm, onClose }: Props) {
  ...
  const reset = () => { setOutType('K'); setSeq([]); };
    ...
    <div className={`ov${open ? ' open' : ''}`} id="ov-strikeout">
      <div className="ov-card" style={{ minWidth: 320 }}>
        <div className="modal-title">
          삼진 결과 — <span style={{ color: 'var(--text3)', fontSize: 11 }}>
        ...
        <div style={{ padding: '10px 14px 6px', borderBottom: '1px solid var(--border2)' }}>
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: needsFielder ? 10 : 0 }}>
            <button style={btnSel(outType === 'K')}   onClick={() => { setOutType('K');   setSeq([]); }}>K  스트라이크 아웃</button>
            <button style={btnSel(outType === 'KT')}  onClick={() => { setOutType('KT');  setSeq([]); }}>KT  태그 아웃</button>
            <button style={btnSel(outType === 'KSG')} onClick={() => { setOutType('KSG'); setSeq([]); }}>K송구  아웃</button>
          </div>
          ...
          {needsFielder && (
            <div>
              <div style={{ fontSize: 10, color: 'var(--text3)', marginBottom: 5 }}>
                ...
                {seq.length > 0 && (
                  <span style={{ marginLeft: 8, fontFamily: 'monospace', color: 'var(--blue)', fontWeight: 700 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 4 }}>
                {[1,2,3,4,5,6,7,8,9].map((pos) => {
                  ...
                  return (
                    <button key={pos} onClick={() => addPos(pos)} style={{
                      padding: '5px 2px', fontSize: 10, fontWeight: 700, borderRadius: 3, cursor: 'pointer',
              ...
              <div style={{ display: 'flex', gap: 6, marginTop: 5 }}>
                ...
                {result && (
                  <span style={{ marginLeft: 4, fontFamily: 'monospace', fontWeight: 700, color: 'var(--red)', fontSize: 13, alignSelf: 'center' }}>
                    {result}
                  </span>
                )}
              </div>
        ...
        <div style={{ padding: '10px 14px' }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--text3)', marginBottom: 6 }}>낫아웃 진루</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {[
              ...
              <button key={code} onClick={() => handleConfirm(code)} style={{
                display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                padding: '8px 14px', fontWeight: 700, fontSize: 12, borderRadius: 3, cursor: 'pointer',
                border: `2px solid ${color}`, background: '#fff', color: 'var(--text)',
              ...
              onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = '#fff'; (e.currentTarget as HTMLButtonElement).style.color = 'var(--text)'; }}
              >
