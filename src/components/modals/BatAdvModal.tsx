...
interface DefRow { pos: number; assist: boolean; putout: boolean; error: boolean; }
...
export default function BatAdvModal({
  ...
  const tryAutoHit = (
    ...
    const hd: HitData = { zone, dirRow: row, dirCol: col, ballType: toBallTypeShort(ball), deflection: defl, bases };
    onSelectHit(hd);
    // 자동확정 없음 — 확인 버튼으로만 처리
  };
  ...
  const handleDir = (row: number, col: number) => {
    ...
    setSelDirRow(row); setSelDirCol(col);
  ...
  const pick = (v: string) => {
    setPendingResult(v);
    setDefSeq([]);
    setSelDirRow(null); setSelDirCol(null);
    setSelBall(null); setDeflection(false);
  };
  ...
  // ── 닫기 ─────────────────────────────────────────────────────────────────
  const handleClose = () => {
    setPendingResult(null); setDefSeq([]);
    setSelDirRow(null); setSelDirCol(null);
    setSelBall(null); setDeflection(false);
    set장외홈런(false); setGhrDist(110); setGhrH(0); setGhrM(0); set연결동작(false);
    onClose();
  };
  ...
  const zone = defSeq[defSeq.length - 1]?.pos ?? null;
  const hitReady = pendingResult && NEEDS_HIT.has(pendingResult) && zone !== null && selDirRow !== null && selDirCol !== null && selBall !== null;

  if (!open) return null;

  return (
    <div className="ov open" id="ov-bat-adv" onClick={(e) => { if (e.target === e.currentTarget) handleClose(); }}>
      <div style={{
        ...
        {/* ── 좌측: 필드 + 수비 테이블 ── */}
        <section style={{ borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{ background: 'var(--panel2)', borderBottom: '1px solid var(--border)', padding: '8px 10px', fontWeight: 700, fontSize: 14 }}>
            수비 + 타구 정보
          </div>

          {/* 필드 + 방향 그리드 */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 130px', gap: 14, alignItems: 'start', padding: 10, borderBottom: '1px solid var(--border2)' }}>
            <div>
              <div style={{ fontSize: 11, color: '#5b6777', marginBottom: 6, fontWeight: 600 }}>  
              </div>
              <div style={{ background: '#f3f5f7', display: 'flex', justifyContent: 'center' }}>
                <svg viewBox="0 0 260 210" style={{ width: '100%', maxWidth: 400, height: 'auto' }}>
                  ...
                  {FPOS.map(({ key, x, y }) => {
                    ...
                    return (
                      <g key={key} style={{ cursor: pendingResult ? 'pointer' : 'default' }}
                        onClick={(e) => { e.stopPropagation(); handlePosClick(key); }}>
                        ...
                        <text x={x} y={y + 6} textAnchor="middle" fontSize="16" fontWeight="700" fill="#fff" pointerEvents="none">{key}</text>
                      </g>
                    );
                  })}
                </svg>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, paddingTop: 22 }}>
              <div>
                <div style={{ fontSize: 11, color: '#5b6777', marginBottom: 5, fontWeight: 600 }}>타구 방향</div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 36px)', gridTemplateRows: 'repeat(3, 36px)', gap: 3 }}>
                  {[0, 1, 2].flatMap((row) =>
                    [0, 1, 2].map((col) => {
                      ...
                      if (isCenter) return (
                        <div key={key} style={{ width: 36, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--panel2)', border: '1px solid var(--border2)', fontSize: 12, fontWeight: 900, color: zone ? 'var(--blue)' : 'var(--text3)', fontFamily: 'monospace' }}>
                          {zone ?? '—'}
                        </div>
                      );
                      return (
                        <button key={key} onClick={() => handleDir(row, col)}
                          disabled={!pendingResult || !NEEDS_HIT.has(pendingResult)}
                          style={{ width: 36, height: 36, padding: 0, border: `1px solid ${isSel ? '#2563eb' : 'var(--border)'}`, background: isSel ? '#2563eb' : '#f7f9fb', color: isSel ? '#fff' : '#444', fontSize: 18, cursor: 'pointer', borderRadius: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: (!pendingResult || !NEEDS_HIT.has(pendingResult)) ? 0.3 : 1 }}>
                          {DIR_LABEL[key]}
                        </button>
                      );
                    })
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {(['땅볼', '뜬공', '라이너'] as BallType[]).map((b) => (
                  <label key={b} style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', fontSize: 12, opacity: (!pendingResult || !NEEDS_HIT.has(pendingResult)) ? 0.3 : 1 }}>
                    <input type="radio" name="batadv_ball" checked={selBall === b}
                      disabled={!pendingResult || !NEEDS_HIT.has(pendingResult)}
                      onChange={() => handleBall(b)} style={{ accentColor: 'var(--blue)' }} />
                    {b}
                  </label>
                ))}
              </div>
          ...
          {(!pendingResult || !NEEDS_HIT.has(pendingResult)) && (
            <div style={{ padding: 10, flex: 1, overflow: 'auto' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8, fontWeight: 700, fontSize: 13 }}>
                <span>수비 리스트{defSeqLabel ? <span style={{ color: 'var(--blue)', marginLeft: 6 }}>{defSeqLabel}</span> : ''}</span>
                {defSeq.length > 0 && (
                  <button onClick={() => setDefSeq([])} style={{ fontSize: 10, padding: '2px 6px', border: '1px solid var(--border)', background: '#fff', cursor: 'pointer', borderRadius: 2 }}>초기화</button>
                )}
              </div>
              {defSeq.length > 0 ? (
                <table style={{ width: '100%', borderCollapse: 'collapse', background: '#fff' }}>
                  <thead>
                    <tr>{['수비', '선수명', '보살', '자살', '실책'].map((h) => (
                      <th key={h} style={{ background: '#dce7f4', border: '1px solid var(--border2)', padding: '5px 4px', fontWeight: 700, fontSize: 11, textAlign: 'center' }}>{h}</th>
                    ))}</tr>
                  </thead>
                  <tbody>
                    {defSeq.map((row, i) => {
                      ...
                      return (
                        <tr key={i}>
                          <td style={{ border: '1px solid var(--border2)', padding: '5px 4px', textAlign: 'center', fontSize: 12 }}>{POS_NAME[row.pos]}</td>
                          <td style={{ border: '1px solid var(--border2)', padding: '5px 6px', fontSize: 12 }}>{player?.name || `#${row.pos}`}</td>
                          <td style={{ border: '1px solid var(--border2)', padding: '5px 4px', textAlign: 'center' }}><input type="checkbox" checked={row.assist}  onChange={() => toggle('assist')}  /></td>
                          <td style={{ border: '1px solid var(--border2)', padding: '5px 4px', textAlign: 'center' }}><input type="checkbox" checked={row.putout} onChange={() => toggle('putout')} /></td>
                          <td style={{ border: '1px solid var(--border2)', padding: '5px 4px', textAlign: 'center' }}><input type="checkbox" checked={row.error}  onChange={() => toggle('error')}  /></td>
                        </tr>
                      );
                    })}
                  </tbody>
                ...
                <div style={{ color: 'var(--text3)', fontSize: 11, padding: '8px 0' }}>
                  {pendingResult
                    ? NEEDS_FIELDER.has(pendingResult) ? '실책 수비수를 클릭하면 바로 기록됩니다'
          ...
          {/* 하단 확인/취소 */}
          <div style={{ display: 'flex', justifyContent: 'center', gap: 14, padding: '12px 10px 14px', borderTop: '1px solid var(--border2)' }}>
            {pendingResult && !NEEDS_HIT.has(pendingResult) && (
              <button className="btn-ok" onClick={handleConfirm}
                disabled={NEEDS_FIELDER.has(pendingResult) && defSeq.length === 0}
                style={{ opacity: (NEEDS_FIELDER.has(pendingResult) && defSeq.length === 0) ? 0.4 : 1 }}>
                확인
              </button>
            )}
            {pendingResult && NEEDS_HIT.has(pendingResult) && (
              <button className="btn-ok" onClick={() => {
                if (!hitReady) return;
                const hd: HitData = { zone: zone!, dirRow: selDirRow!, dirCol: selDirCol!, ballType: toBallTypeShort(selBall!), deflection, bases: basesForType(pendingResult) };
                onAutoConfirm('HIT', undefined, hd);
              }} disabled={!hitReady} style={{ opacity: hitReady ? 1 : 0.4 }}>
        ...
        <section style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', padding: '5px 10px', borderBottom: '1px solid var(--border2)' }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, cursor: 'pointer' }}>
              <input type="checkbox" checked={연결동작} onChange={(e) => set연결동작(e.target.checked)} style={{ accentColor: 'var(--blue)' }} />
              연결동작
            </label>
          </div>
            ...
            <div style={{ borderRight: '1px solid var(--border2)' }}>
              <div style={{ padding: '10px', borderBottom: '1px solid var(--border2)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <Btn label="◇ 홈런" sel={s('HR')} onClick={() => pick('HR')} style={{ flex: 1 }} />
                    <label style={{ display: 'flex', alignItems: 'center', gap: 3, fontSize: 10, cursor: 'pointer', whiteSpace: 'nowrap' }}>
                      <input type="checkbox" checked={장외홈런} onChange={(e) => set장외홈런(e.target.checked)} style={{ accentColor: 'var(--blue)' }} />
                      장외홈런
                    </label>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    <Btn label="◇G 그라운드" sel={s('GHR')} onClick={() => pick('GHR')} style={{ flex: 1 }} />
                    <select value={ghrDist} onChange={(e) => setGhrDist(Number(e.target.value))}
                      style={{ fontSize: 10, border: '1px solid var(--border)', borderRadius: 2, padding: '2px 2px' }}>
                      {GHR_DISTS.map((d) => <option key={d} value={d}>{d}</option>)}
                    </select>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4, paddingLeft: 4 }}>
                    <span style={{ fontSize: 9, color: 'var(--text3)', whiteSpace: 'nowrap' }}>홈런 시각</span>
                    <input type="number" value={ghrH} min={0} max={23} onChange={(e) => setGhrH(Number(e.target.value))}
                      style={{ width: 36, fontSize: 10, border: '1px solid var(--border)', borderRadius: 2, padding: '1px 3px', textAlign: 'center' }} />
                    <span style={{ fontSize: 10 }}>:</span>
                    <input type="number" value={ghrM} min={0} max={59} onChange={(e) => setGhrM(Number(e.target.value))}
                      style={{ width: 36, fontSize: 10, border: '1px solid var(--border)', borderRadius: 2, padding: '1px 3px', textAlign: 'center' }} />
                  </div>
              ...
              <div style={{ padding: '10px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4 }}>
                  ...
                  <Btn label="□ 선행주자 아웃" sel={s('선행주자아웃')} onClick={() => pick('선행주자아웃')} />
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '12px 10px 14px', borderTop: '1px solid var(--border2)' }}>
            <button className="btn-cancel" onClick={handleClose}>취소</button>
          </div>
        </section>
      </div>
