    ...
    <div className="ov open" id="ov-def-list">
      <div className="ov-card" style={{ minWidth: 560 }}>
        ...
        >
          ...
          >
            <input
              type="checkbox"
              checked={수비시정}
              onChange={(e) => set수비시정(e.target.checked)}
        ...
        <div style={{ padding: '10px 12px' }}>
          {positions.length === 0 ? (
            <div
              style={{ padding: '16px', textAlign: 'center', color: 'var(--text3)', fontSize: 12 }}
            >
              수비 수비수가 없습니다.
            </div>
          ) : (
                ...
                {rows.map((row, i) => {
                  ...
                  return (
                    <tr key={i} style={{ background: rowBg }}>
                      <td style={{ ...tdStyle, background: selBg, fontWeight: 700 }}>
                      ...
                      <td style={tdStyle}>
                        <select
                          value={row.throwDir}
                          onChange={(e) => update(i, { throwDir: e.target.value })}
                          style={{
                            fontSize: 10,
                            border: '1px solid var(--border2)',
                            borderRadius: 2,
                      ...
                      <td style={tdStyle}>
                        <select
                          value={row.recvDir}
                          onChange={(e) => update(i, { recvDir: e.target.value })}
                          style={{
                            fontSize: 10,
                            border: '1px solid var(--border2)',
                            borderRadius: 2,
