...
export default function Calendar({ selectedDate, onPick, onClose }: Props) {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    cells.push(
      <button key={d} className={cls} onClick={() => onPick(year, month, d)}>{d}</button>
    );
  }
  ...
  return (
    <div className="cal-wrap open" ref={ref} onClick={(e) => e.stopPropagation()}>
      <div className="cal-header">
        ...
        <span style={{ fontSize: 12, fontWeight: 700, flex: 1, textAlign: 'center' }}>{year}년 {month}월</span>
        <button onClick={nextMonth}>›</button>
        <button onClick={() => setYear((y) => y + 1)}>»</button>
      </div>
      <div className="cal-grid">
        {WEEK.map((d) => <div key={d} className="cal-day-hd">{d}</div>)}
        {cells}
      </div>
    </div>
  );
